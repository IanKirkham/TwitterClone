...
public class GetFolloweesCountTask extends GetFollowCountTask {
    /**
     * Creates an instance.
     *
     ...
     */
    public GetFolloweesCountTask(UserPresenter presenter, Observer observer) {
    ...
    protected GetCountResponse doInBackground(GetCountRequest... getCountRequests) {
        ...
        try {
            response = presenter.getFolloweeCount(getCountRequests[0]);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return response;
    ...
    protected void onPostExecute(GetCountResponse getCountResponse) {
        observer.updateFolloweeCount(getCountResponse.getCount());
    }
}